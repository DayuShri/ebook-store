        ...
        <div class="grid lg:grid-cols-3 gap-8">
                ...
                @foreach($cart['items'] as $item)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <a href="{{ route('books.show', $item['book_id']) }}" class="font-semibold text-gray-900 hover:text-primary-600 line-clamp-2">
                                {{ $item['book']['title'] }}
                            </a>
                            ...
                            <div class="mt-2">
                                @if(!empty($item['book']['discount_percentage']) && $item['book']['discount_percentage'] > 0)
                                    ...
                                    <span class="text-sm text-gray-400 line-through ml-2">Rp {{ number_format($item['book']['price'], 0, ',', '.') }}</span>
                                @else
                                    <span class="font-bold text-primary-600">Rp {{ number_format($item['subtotal'], 0, ',', '.') }}</span>
                                @endif
                            </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
                    ...
                    <div class="mb-6">
                        ...
                        @else
                            <form action="{{ route('cart.voucher') }}" method="POST" class="flex gap-2">
                                ...
                                <button type="submit" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-200 transition-colors text-sm">
                                    Terapkan
                                </button>
                            </form>
                        @endif
                    ...
                    {{-- Checkout Button --}}
                    <a href="{{ route('checkout') }}" class="mt-6 block w-full bg-primary-600 text-white text-center py-3 rounded-lg font-semibold hover:bg-primary-700 transition-colors">
                        Lanjut ke Pembayaran
                    </a>

                    <a href="{{ route('books.index') }}" class="mt-3 block w-full text-center text-gray-600 hover:text-gray-900 text-sm">
                        Lanjut Belanja
                    </a>
                </div>
            </div>
