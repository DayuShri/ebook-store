...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900">
            {{ $pageTitle ?? (!empty($searchQuery) ? 'Hasil Pencarian: "' . $searchQuery . '"' : 'Katalog Buku') }}
        </h1>

        @php
            // Support paginator atau array
            $isPaginator = is_object($books) && method_exists($books, 'total');
            $shownCount  = $isPaginator ? $books->count() : (is_array($books) ? count($books) : 0);
            $totalCount  = $isPaginator ? $books->total() : $shownCount;
        @endphp

        <p class="text-gray-600 mt-1">
            @if($totalCount > 0)
                Menampilkan {{ $shownCount }} dari {{ $totalCount }} buku
            @else
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1">
            ...
            <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center space-x-2">
                    <button type="button" class="p-2 bg-primary-100 text-primary-700 rounded-lg" title="Grid View">
            ...
            @else
                <div class="text-center py-16">
                    ...
                    <a href="{{ route('books.index') }}"
                       class="mt-6 inline-block bg-primary-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-primary-700 transition-colors">
                        Reset Filter
                    </a>
                </div>
            @endif
