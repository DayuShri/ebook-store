            ...
            @foreach($wishlistItems as $item)
                ...
                @if($book)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group relative">
                        ...
                        <form action="{{ route('wishlist.remove', $bookId) }}" method="POST" class="absolute top-2 right-2 z-10">
                            @csrf
                            @method('DELETE')
                        ...
                        {{-- Book Cover --}}
                        <a href="{{ route('books.show', $bookId) }}" class="block relative aspect-[2/3] overflow-hidden">
                        ...
                        {{-- Book Info --}}
                        <div class="p-4">
                            <a href="{{ route('books.show', $bookId) }}" class="font-semibold text-gray-900 hover:text-primary-600 line-clamp-2">
                                {{ $book['title'] }}
                            </a>
                            ...
                            <div class="mt-2">
                                ...
                                @else
                                    <span class="font-bold text-primary-600">Rp {{ number_format($book['price'] ?? 0, 0, ',', '.') }}</span>
                                @endif
                            </div>
                ...
                @else
                    {{-- Book was deleted from catalog --}}
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group relative opacity-60">
                        {{-- Remove from Wishlist --}}
                        <form action="{{ route('wishlist.remove', $bookId) }}" method="POST" class="absolute top-2 right-2 z-10">
                            @csrf
                            @method('DELETE')
                        ...
                        {{-- Placeholder Cover --}}
                        <div class="block relative aspect-[2/3] overflow-hidden">
                        ...
                        {{-- Unavailable Info --}}
                        <div class="p-4">
                            <span class="font-semibold text-gray-500 line-clamp-2">Buku tidak tersedia</span>
                            <p class="text-sm text-gray-400 mt-1">Buku ini telah dihapus dari katalog</p>
                        </div>
                    </div>
                @endif
            @endforeach
